#!/usr/bin/env python3
"""
FIXED binCraft decoder - corrects lat/lon swap and altitude/rate swap
Based on readsb struct binCraft definition
"""
import struct
import math
...
def binCraftReader(file,zstd_compressed=False):
    ...
    for off in range(stride,len(data),stride):
        ...
        # FIXED: lat/lon were swapped in original decoder
        # According to readsb aircraft.h struct:
        #   int32_t lon at offset 8 (s32[2])
        #   int32_t lat at offset 12 (s32[3])
        ac['lon'] = s32[2] / 1e6  # was: ac['lat'] = s32[2] / 1e6
        ac['lat'] = s32[3] / 1e6  # was: ac['lon'] = s32[3] / 1e6

        # FIXED: altitude and rate indices were swapped!
        # According to readsb aircraft.h struct:
        #   int16_t baro_rate at offset 16 (s16[8])
        #   int16_t geom_rate at offset 18 (s16[9])
        #   int16_t baro_alt at offset 20 (s16[10])
        #   int16_t geom_alt at offset 22 (s16[11])
        ac['baro_rate'] = s16[8] * 8   # was: ac['alt_baro'] = s16[8] * 25
        ac['geom_rate'] = s16[9] * 8   # was: ac['alt_geom'] = s16[9] * 25
        ac['alt_baro'] = s16[10] * 25  # was: ac['baro_rate'] = s16[10] * 8
        ac['alt_geom'] = s16[11] * 25  # was: ac['geom_rate'] = s16[11] * 8
