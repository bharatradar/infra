// --- Frontend Config (loaded from /command_center/api/config) ---
let FRONTEND_CONFIG = {
...
async function loadFrontendConfig() {
    try {
        const res = await fetch('/command_center/api/config');
        if (res.ok) {
...
async function loadFilterOptions() {
    try {
        const res = await fetch('/command_center/api/filters');
...
async function loadAircraftDB() {
    try {
        const res = await fetch('/command_center/api/aircraft-db?hex_id=000000');
        if (!res.ok) return;
        const data = await res.json();
...
async function fetchAircraftTypes(hexIds) {
    ...
    try {
        ...
        const res = await fetch(`/command_center/api/aircraft-db?batch=${batch}`);
        const data = await res.json();
...
async function openDrillDownModal(type, targetCode, targetDisplay) {
    ...
    try {
        if (type === 'turnaround') {
            const res = await fetch(`/command_center/api/drilldown/turnaround?target_airline=${targetCode}&airport=${f.airport}`);
            const data = await res.json();
            ...
            const res = await fetch(`/command_center/api/drilldown/otp?target_airline=${targetCode}&airport=${f.airport}`);
            const data = await res.json();
            ...
            const res = await fetch(`/command_center/api/drilldown/fleet?hex_id=${targetCode}`);
            const data = await res.json();
            ...
            const res = await fetch(`/command_center/api/drilldown/safety?target_date=${targetCode}`);
            const data = await res.json();
            ...
            const res = await fetch(`/command_center/api/drilldown/cdo?target_airline=${targetCode}`);
            const data = await res.json();
            ...
            const res = await fetch(`/command_center/api/drilldown/route?origin=${parts[0]}&destination=${parts[1]}`);
            const data = await res.json();
            ...
            const res = await fetch(`/command_center/api/drilldown/demand?hour_bucket=${targetCode}&airport=${f.airport}`);
            const data = await res.json();
            ...
            const res = await fetch(`/command_center/api/drilldown/altitude?band=${encodeURIComponent(targetCode)}&airline=${f.airline}&airport=${f.airport}`);
            const data = await res.json();
...
async function openForensicsModal(hexId, callsign) {
    ...
    try {
        const aiRes = await fetch(`/command_center/api/ai/audit?hex_id=${hexId}&callsign=${callsign}`);
    ...
    // 2. Fetch Telemetry
    try {
        const res = await fetch(`/command_center/api/telemetry/track?hex_id=${hexId}`);
        const data = await res.json();
        if (data.length === 0) {
...
async function fetchATC() {
    try {
        ...
        if (WS_USE_FOR_ATC && wsFlightsData.length > 0) {
            ...
            const resMap = await fetch(`/command_center/api/atc/live?airline=${f.airline}&airport=${f.airport}`);
        ...
        try {
            const resBands = await fetch(`/command_center/api/atc/bands?airline=${f.airline}&airport=${f.airport}`);
        ...
        try {
            const resAnom = await fetch(`/command_center/api/atc/anomalies?airline=${f.airline}&airport=${f.airport}`);
...
async function fetchDelayPredictions() {
    try {
        ...
        const resRoute = await fetch('/command_center/api/delay/route_otp?limit=50');
        ...
        const resAirport = await fetch('/command_center/api/delay/airports');
...
async function fetchSchedules() {
    ...
    try {
        ...
        const resArr = await fetch(`/command_center/api/ops/${arrEndpoint}?airport=${f.airport}&direction=ARRIVALS${dateParam}`);
        rawSchedules.arr = await resArr.json();
        applyLocalFilters('arr'); 

        const depEndpoint = terminalModes.dep === 'board' ? 'schedules' : 'logs';
        const resDep = await fetch(`/command_center/api/ops/${depEndpoint}?airport=${f.airport}&direction=DEPARTURES${dateParam}`);
...
async function fetchAIOperations() {
    try {
        const resEnrich = await fetch('/command_center/api/ai/operations/enrichment');
        ...
        const resInsights = await fetch('/command_center/api/ai/operations/insights');
...
async function fetchOps() {
    ...
    const f = getFilters();
    try {
        const resSq = await fetch(`/command_center/api/ops/squatters?airport=${f.airport}&airline=${f.airline}`);
        ...
        const resTurn = await fetch(`/command_center/api/ops/turnarounds?airport=${f.airport}&airline=${f.airline}`);
        ...
        const resDem = await fetch(`/command_center/api/ops/runway_demand?airport=${f.airport}`);
        ...
        const resFleet = await fetch(`/command_center/api/ops/fleet_utilization?airline=${f.airline}`);
        ...
        const resOtp = await fetch(`/command_center/api/ops/otp?airport=${f.airport}&airline=${f.airline}`);
...
async function fetchExec() {
    ...
    const f = getFilters();
    try {
        const resSafe = await fetch('/command_center/api/exec/safety');
        ...
        const resRoutes = await fetch('/command_center/api/exec/routes');
        ...
        const resCdo = await fetch('/command_center/api/exec/approach_efficiency');
        ...
        const resUns = await fetch(`/command_center/api/exec/unscheduled?airport=${f.airport}`);
        ...
        const resTrain = await fetch(`/command_center/api/exec/training?airport=${f.airport}`);
...
async function sendChatMessage() {
    ...
    try {
        ...
        const res = await fetch('/command_center/api/chat', {
            method: 'POST',
...
async function enableWebPush() {
    ...
    try {
        ...
        const swReg = await navigator.serviceWorker.register('/command_center/sw/service-worker.js');
        
        // Fetch your server's VAPID Public Key
        const pkRes = await fetch('/command_center/api/push/public_key');
        ...
        const subRes = await fetch('/command_center/api/push/subscribe', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function fetchRadarData() {
    ...
    try {
        const res = await fetch('/command_center/api/atc/live?airline=ALL&airport=ALL');
