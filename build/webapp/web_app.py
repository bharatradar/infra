...
PUBLIC_ROUTES = [
    ...
    "/auth/",
    "/command_center/auth/",
    "/command_center/login",
    "/command_center/logout",
    "/static/",
    "/favicon.ico",
    ...
    "/command_center/login",
    "/command_center/auth/google",
    "/command_center/auth/callback",
    # ATC radar data (no auth for dashboard display)
    "/command_center/api/atc/live",
    "/command_center/api/filters",
    "/command_center/static/",
    "/favicon.ico",
    # API docs (public access)
    "/command_center/docs",
    "/command_center/redoc",
    "/command_center/openapi.json",
    "/docs",
    "/redoc",
    "/openapi.json",
]
...
async def _feeder_health_monitor_task():
    ...
    while True:
        try:
            ...
            # Read receivers.json
            with open('/run/readsb/receivers.json', 'r') as f:
                data = json.load(f)
...
# Manually serve docs at /command_center/* so they work behind nginx/FRP proxy
@app.get("/command_center/docs", include_in_schema=False)
async def custom_swagger_ui_html():
    return get_swagger_ui_html(
        openapi_url="/command_center/openapi.json",
        title=app.title + " - Swagger UI",
        oauth2_redirect_url="/command_center/docs/oauth2-redirect",
    )

@app.get("/command_center/docs/oauth2-redirect", include_in_schema=False)
async def swagger_ui_redirect():
    # FastAPI's built-in oauth2 redirect handler
    from fastapi.openapi.docs import get_swagger_ui_oauth2_redirect_html
    return get_swagger_ui_oauth2_redirect_html()

@app.get("/command_center/openapi.json", include_in_schema=False)
async def get_openapi_endpoint():
    return JSONResponse(
        get_openapi(title=app.title, version=app.version, routes=app.routes)
    )

@app.get("/command_center/redoc", include_in_schema=False)
...
async def auth_middleware(request: Request, call_next):
    ...
    if not is_session_valid(request):
        ...
        elif path.startswith("/command_center"):
            return RedirectResponse(url="/command_center/login")
        else:
...
app.mount("/command_center/static", StaticFiles(directory="static"), name="static")
app.mount("/command_center/sw", StaticFiles(directory="static"), name="sw")

@app.get("/")
async def root_redirect(): return RedirectResponse(url="/command_center/")
...
async def serve_landing_page(request: Request):
    ...
    if is_session_valid(request):
        return RedirectResponse(url="/command_center/dashboard")
    return RedirectResponse(url="/command_center/login")

@app.get("/command_center/login")
@app.get("/command_center/login/")
...
@app.get("/command_center/logout")
@app.get("/command_center/logout/")
async def logout():
    response = RedirectResponse(url="/command_center/login")
    response.delete_cookie(AUTH_COOKIE_NAME, path="/")
    return response

@app.get("/command_center/dashboard")
@app.get("/command_center/dashboard/")
...
@app.get("/command_center/testmodel")
@app.get("/command_center/testmodel/")
...
@app.get("/command_center/api-access")
@app.get("/command_center/api-access/")
...
@app.get("/command_center/feeders/map")
@app.get("/command_center/feeders/map/")
...
@app.get("/command_center/feeders/account/{feeder_id}")
...
@app.get("/command_center/feeders/health")
@app.get("/command_center/feeders/health/")
...
async def auth_google_root_proxy():
    ...
    async with httpx.AsyncClient() as client:
        response = await client.get("http://127.0.0.1:8000/auth/google", follow_redirects=False)
...
async def auth_callback_root_proxy(code: str):
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(f"http://127.0.0.1:8000/auth/callback?code={code}", follow_redirects=False)
            if response.status_code == 200:
                ...
                redirect = RedirectResponse(url="/command_center/dashboard", status_code=302)
                redirect.set_cookie(
                ...
                return RedirectResponse("/command_center/login?auth=error", status_code=302)
    except Exception as e:
        logger.error(f"Auth callback error: {e}")
        return RedirectResponse("/command_center/login?auth=error", status_code=302)

@app.get("/command_center/auth/google")
...
@app.get("/command_center/auth/callback")
async def auth_callback_proxy(code: str):
    import httpx
    from datetime import datetime, timedelta
    try:
        async with httpx.AsyncClient() as client:
...
@app.post("/command_center/raga_radar_chat")
...
# ATC Endpoints
@app.get("/command_center/api/filters")
async def api_filters():
    return await web_app_db.fetch_filter_options(db_pool)

@app.get("/command_center/api/atc/live")
...
@app.get("/command_center/api/aircraft/all")
...
@app.get("/command_center/api/atc/bands")
...
@app.get("/command_center/api/atc/anomalies")
...
# Ops Endpoints
@app.get("/command_center/api/ops/squatters")
...
@app.get("/command_center/api/ops/turnarounds")
...
@app.get("/command_center/api/drilldown/turnaround")
...
@app.get("/command_center/api/ops/schedules")
...
@app.get("/command_center/api/ops/logs")
...
@app.get("/command_center/api/drilldown/demand")
...
@app.get("/command_center/api/drilldown/altitude")
...
# ---------------------------------------------------------------------
@app.get("/command_center/api/aircraft-db")
...
@app.post("/command_center/api/chat")
...
@app.get("/command_center/api/config")
...
@app.get("/command_center/api/history/arrivals")
...
@app.get("/command_center/api/history/departures")
...
@app.get("/command_center/api/history/flights")
...
@app.get("/command_center/api/history/route/{origin}/{dest}")
...
@app.get("/command_center/api/history/stats")
...
@app.get("/command_center/api/delay/predict")
...
@app.get("/command_center/api/delay/airline_otp")
...
@app.get("/command_center/api/delay/route_otp")
...
@app.get("/command_center/api/delay/airports")
...
@app.post("/command_center/api/feeders/register")
...
@app.get("/command_center/api/auth/me")
...
@app.post("/command_center/api/auth/me")
...
@app.get("/command_center/api/keys")
...
@app.post("/command_center/api/keys")
...
@app.delete("/command_center/api/keys/{key_id}")
...
@app.get("/command_center/api/feeders/leaderboard")
...
@app.get("/command_center/api/feeders/profile/{feeder_id}")
...
@app.post("/command_center/api/feeders/stats")
...
@app.get("/command_center/api/feeders/coverage/gaps")
...
@app.get("/command_center/api/feeders/tiers/summary")
...
@app.get("/command_center/api/feeders/my-stations")
...
@app.post("/command_center/api/feeders/{feeder_id}/claim-station")
...
@app.post("/command_center/api/feeders/{feeder_id}/update")
...
@app.get("/command_center/api/feeders/{feeder_id}/stats")
...
@app.get("/command_center/api/feeders/health")
...
@app.get("/command_center/api/feeders/{feeder_id}/health")
...
@app.post("/command_center/api/feeders/{feeder_id}/settings")
...
@app.get("/command_center/api/feeders/map")
...
@app.get("/command_center/api/feeders/{feeder_id}/history")
...
@app.get("/command_center/api/feeders/{feeder_id}/peers")
...
@app.get("/command_center/api/feeders/coverage/heatmap")
...
@app.api_route("/command_center/api/feeders/{feeder_id}/view/{path:path}", methods=["GET", "POST", "HEAD"])
async def proxy_tar1090(feeder_id: int, path: str, request: Request):
    """
    Reverse-proxy tar1090 for a specific feeder.
    Browser sees only /command_center/api/feeders/<id>/view/...
    ...
    try:
        ...
        proxy_base = f"/command_center/api/feeders/{feeder_id}/view/"
...
@app.get("/command_center/api/feeders/{feeder_id}/map")
async def get_feeder_map_page(feeder_id: int, request: Request):
    ...
    try:
        async with db_pool.acquire() as conn:
            ...
            if not feeder or feeder['user_email'] != user_email:
                return RedirectResponse(url="/command_center/profile")

            if not feeder.get('station_uuid'):
                return RedirectResponse(url="/command_center/profile")

        return RedirectResponse(
            url=f"/command_center/api/feeders/{feeder_id}/view/",
            status_code=302
        )
    except Exception as e:
