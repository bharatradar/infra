...
# 🌟 NEW: Persistent MCP Client Connection
MCP_SESSION = None
MCP_EXIT_STACK = None
...
async def init_client_state():
    ...
    # 4. Start the MCP Client Pipeline
    python_path = getattr(Config, 'PYTHON_PATH', 'python')
    ...
    try:
        ...
        logger.info("✅ Persistent MCP Client connected to mcp_server.py")
    except Exception as e:
        logger.error(f"⚠️ Failed to start MCP client: {e}")

async def close_client_state():
    if DB_POOL: await DB_POOL.close()
    if REDIS_POOL: await REDIS_POOL.aclose() 
    if MCP_EXIT_STACK: await MCP_EXIT_STACK.aclose()
...
async def execute_tool_via_mcp(tool_name: str, params: dict) -> str:
    """Proxies the request to the isolated MCP server running locally."""
    if not MCP_SESSION:
        return "⚠️ Backend MCP Server is currently disconnected."
    
    if not tool_name or tool_name is None:
        return "⚠️ No tool specified for execution."
    ...
    try:
        ...
        return f"⚠️ MCP Execution Error on Backend: {type(e).__name__}: {str(e) or 'Unknown error'}"
                ...
                if provider == "local_gguf":
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.post(
                            ...
                            if resp.status == 200:
                                data = await resp.json()
                                raw_content = data["choices"][0]["text"].strip()
                            else:
                                continue
                else:
    ...
    for key, val in params.items():
        if isinstance(val, str):
            if key in ["airport", "airport_code", "code", "origin", "destination", "partner_airport"]:
                if val.lower() in ROUTER.city_map: params[key] = ROUTER.city_map[val.lower()]
