...
# 🌟 NEW: Direct GGUF Model Configuration
    ...
    PROXY_URL = "http://192.168.100.19:8888"
    ...
    # --- API Server for OAuth ---
    API_INTERNAL_URL = _env("API_INTERNAL_URL", "http://127.0.0.1:8000")
    ...
    # ==========================================
    # Polling Intervals (All configurable)
    # ==========================================
    # Main radar data fetch interval (seconds)
    RADAR_FETCH_INTERVAL_SEC = 10

    # WebSocket broadcast interval (seconds)
    WEBSOCKET_BROADCAST_INTERVAL_SEC = 1

    # Gap filler / enrichment worker interval (seconds)
    GAP_FILLER_INTERVAL_SEC = 10

    # Enrichment pre-fetch sleep between flights (seconds)
    ENRICHMENT_FETCH_DELAY_SEC = 2

    # Background janitor cleanup interval (seconds)
    JANITOR_INTERVAL_SEC = 900  # 15 minutes

    # Forensic janitor AI interval (seconds)
    FORENSIC_JANITOR_INTERVAL_SEC = 900  # 15 minutes

    # APOC watchdog check interval (seconds)
    WATCHDOG_CHECK_INTERVAL_SEC = 120  # 2 minutes

    # Daily analyst report interval (seconds)
    DAILY_ANALYST_INTERVAL_SEC = 900  # 15 minutes

    # Web app feeder health check interval (seconds)
    FEEDER_HEALTH_CHECK_INTERVAL_SEC = 30
    FEEDER_HEALTH_CHECK_TIMEOUT_SEC = 10
    READSB_RECEIVERS_URL = _env("READSB_RECEIVERS_URL", "http://hub-readsb.bharatradar.svc.cluster.local:150/receivers.json")

    # Web app stats parser interval (seconds)
    STATS_PARSER_INTERVAL_SEC = 120  # 2 minutes

    # Background task startup delays (seconds)
