...
**File:** `infra/build/cortex-webapp/weather_service.py`
...
**File:** `infra/build/cortex-webapp/web_app.py`
...
**Files:** `infra/build/cortex-webapp/static/dashboard.html`, `infra/build/cortex-webapp/static/js/app.js`

- Weather card appears in ATC tab when an airport is selected
...
**File:** `infra/build/cortex-webapp/delay_predictor.py`
...
**File:** `infra/build/cortex-webapp/bot_router_mcp_client.py`
...
- Semantic router handles queries like "weather at Mumbai", "is it foggy in Delhi"
...
| File (relative to infra/) | Action | Purpose |
|------|--------|---------|
| `build/cortex-webapp/weather_service.py` | **NEW** | Core weather fetch + cache + delay + alert logic |
| `build/cortex-webapp/web_app.py` | Modified | `/api/weather/*` routes, import weather_service |
...
Each phase is built, tagged, and deployed to the `feature/openmeteo-integration` branch.
